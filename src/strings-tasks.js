...
function repeatString(str, times) {
  return typeof times === 'number' && times >= 0 ? str.repeat(times) : '';
}
...
function countVowels(str) {
  const vowelMatches = str.match(/[aeiouyAEIOUY]/g);
...
function isPalindrome(str) {
  const cleanedString = str.toLowerCase().replaceAll(/[^a-z]+/g, '');
  ...
  return sentence
    ...
    .reduce(
      (longest, word) =>
        ...
        longest.replace(/[^a-zA-Z]/g, '').length
          ? word
          : longest,
      ''
    );
